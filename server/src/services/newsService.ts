...
const buildNewsQuery = async (user: IUser): Promise<string> => {
  const parts: string[] = []

  // 1. Niche — always included
  if (user.niche) parts.push(user.niche)
  ...
  try {
    ...
    if (creator?.styleFingerprint) {
      const fp = creator.styleFingerprint as any
...
export const fetchNewsByUser = async (user: IUser): Promise<NewsArticle[]> => {
  try {
    ...
    if (articles.length === 0) return getMockNews(user)
...
const getMockNews = (user: IUser): NewsArticle[] => {

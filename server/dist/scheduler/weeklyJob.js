...
exports.startWeeklyJob = void 0;
const node_cron_1 = __importDefault(require("node-cron"));
const User_1 = __importDefault(require("../models/User"));
const Post_1 = __importDefault(require("../models/Post"));
const aiService_1 = require("../services/aiService");
const analyseUserData = async (userId) => {
    try {
...
OUTPUT this exact JSON structure:
{
  ...
  "topicsCovered": [${recentPosts.flatMap(p => p.topicTags).map(t => `"${t}"`).join(', ')}]
        ...
        // Call AI
        const rawCard = await (0, aiService_1.generatePost)(analyserPrompt);
        // Parse JSON
        const clean = rawCard
            .replace(/```json/g, '')
            .replace(/```/g, '')
        ...
        // Save to user
        await User_1.default.findByIdAndUpdate(userId, { intelligenceCard });
...
exports.startWeeklyJob = startWeeklyJob;
